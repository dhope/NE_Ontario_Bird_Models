...
spp <- tibble(english_name = unique(counts$species_name_clean)) |> 
  left_join(naturecounts::meta_species_taxonomy() |> 
...
get_na_pops <- function(spp){
  if(spp %in% napops_spp$Species){
    ...
    calc_values <- function(edr, cr,time_minutes, max_dist){
      ...
      # edr_ha <- edr/100
      # browser()
    tibble(edr,cr,max_dist,time_minutes,
           ...
           q = if_else(inf, 1,edr^2 * (1 - exp(-max_dist^2/edr^2))/max_dist^2),
           o = log(A) + log(p) + log(q))
    ...
    expand_grid(
      ...
      rowwise() |> 
      mutate(qpad = list(calc_values(edr_est_ha, cr, time_minutes , max_dist))) |>
      ungroup() |> 
      dplyr::select(qpad) |> unnest(qpad) |> 
      mutate(spp = spp)
  
  } else{tibble(spp)}
}

na_pops_offsets <- map(unique(spp$species_code), get_na_pops) |> list_rbind() |> 
  left_join(spp |> distinct(english_name, species_code),
            by = join_by(spp == species_code))
  

write_rds(na_pops_offsets ,
          "output/na_pops_offsets.rds")
