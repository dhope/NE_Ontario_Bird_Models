...
spp_code_file <- g("{rds_data_loc}/spp_codes.rds")
  
  #"output/rds/species_codes.rds"
# if (file.exists(spp_code_file)) {
spp_codes <- read_rds(spp_code_file)
# } else {
#   raw_counts <- read_rds(g("{rds_data_loc}/counts.rds"))
#   all_spp <- distinct(raw_counts, common_id, species_name_clean, spp_group)
#   # non_na <- filter(all_spp, !is.na(species_name_clean) & !is.na(common_id))
#   write_rds(all_spp, spp_code_file)
# }
...
ra_buffer <- st_difference(ra_buffer, dplyr::select(
coastal_strip))
...
# 
# 
individual_locs <- distinct(locs, site_id, geometry)
...
train_locs <- test_training_data$train_recordings |>
  ...
  st_as_sf() |>
  dplyr::distinct(site_id, geometry) |>
