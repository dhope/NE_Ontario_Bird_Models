...
exclusion_grid <- st_make_grid(locs, n = c(20, 20)) |>
  ...
  st_filter(locs) |> 
  st_join(locs) |> 
  summarize(n=n(), .by = geometry) |> arrange(desc(n)) |>
...
test_locations <- st_filter(locs, exclusion_grid[exclusion_grid$ID%in%test_blocks$sites_base$ID,])

})

test_locs_ids <- left_join(test_locations, st_drop_geometry(aggregated_locs),
                           by = join_by(site_id_agg))

full_recordings_train <- g("{rds_data_loc}/all_events.rds") |> 
  read_rds() |> 
  filter(location %in% aggregated_locs$location) |> 
  filter(collection %in% c("WildTrax", "ONATLAS3PC") & !location %in% test_locs_ids$location &
